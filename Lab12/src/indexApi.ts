...
import type { Post, PostsState } from './types.js';
...
const searchButton = document.querySelector<HTMLButtonElement>('#search-posts');
const filterInput = document.querySelector<HTMLInputElement>('#filter-input');

// Стан 
let state: PostsState = { status: 'idle' };
let allPosts: Post[] = []; // Зберігаємо всі пости (відомості про ігри) для фільтрації


//Фільтрує ігри за назвою
function filterPosts(posts: Post[], searchTerm: string): Post[] {
  if (!searchTerm.trim()) {
    return posts;
  }
