...
type PageType = 'login' | 'register' | 'unknown';
...
function showError(input: HTMLInputElement | HTMLTextAreaElement, message: string): void {
  ...
  let errorDiv = input.parentElement?.querySelector('.error-message') as HTMLDivElement | null;
...
function showSuccess(input: HTMLInputElement | HTMLTextAreaElement): void {
  input.classList.remove('invalid');
  input.classList.add('valid');
  input.setCustomValidity('');

  const errorDiv = input.parentElement?.querySelector('.error-message') as HTMLDivElement | null;
  if (errorDiv) {
...
function setupRealtimeValidation(): void {
  ...
  if (birthdateInput) {
    const validateBirthdateField = (): void => {
...
function handleRegistration(event: Event): void {
  ...
  for (const radio of Array.from(genderInputs)) {
    if (radio.checked) {
      selectedGender = radio.value as 'male' | 'female';
      break;
    }
  }
 ...
 if (errors.length === 0) {
    ...
    errors.forEach(error => {
      const input = document.getElementById(error.field) as HTMLInputElement | HTMLTextAreaElement;
      if (input) {
...
function handleLogin(event: Event): void {
  ...
  const subscribeRadios = document.getElementsByName('subscribe') as NodeListOf<HTMLInputElement>;

  let selectedSubscribe: 'yes' | 'no' = 'yes';
  for (const radio of Array.from(subscribeRadios)) {
    if (radio.checked) {
      selectedSubscribe = radio.value as 'yes' | 'no';
      break;
    }
  }
  ...
  const registeredUser = loadFromLocalStorage<StoredUserData>('registeredUser');
  ...
  const loginData: CurrentSession = {
    ...registeredUser,
...
function showWelcomeBlock(data: StoredUserData | CurrentSession, source: 'registration' | 'login' = 'registration'): void {
  if (!form) return;

  form.style.display = 'none';

  const formTitle = document.querySelector('body.form-page > h2') as HTMLElement | null;
  if (formTitle) {
...
function checkExistingUser(): void {
  const currentUser = loadFromLocalStorage<CurrentSession>('currentUser');
