...
import { validateName, validateAge, validateEmail, validatePassword, validateUsername, validateBirthdate, validateRegisterForm, validateLoginForm, validateField, formatName, formatEmail, getAgeCategory } from './validator.js';
...
try {
    ...
    console.log('Спіймана помилка (validateName з числом):', error.message);
}
try {
    // @ts-expect-error - тестуємо runtime помилку
    ...
    console.log('Спіймана помилка (validateEmail з масивом):', error.message);
}
// ===== 8. ПРИКЛАД РЕАЛЬНОГО ВИКОРИСТАННЯ =====
console.log('\n📋 8. ПРИКЛАД РЕАЛЬНОГО ВИКОРИСТАННЯ\n');
function processRegistration(rawData) {
